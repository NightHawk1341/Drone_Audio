...
def pipeline_annotation_vers_modele(
    ...
    # Ajoute ces prints dans ton pipeline:
    print(f"Nombre de fichiers audio segmentés: {len(list(segments_dir.glob('train/*.wav')))}")
    print(f"Nombre de labels train: {len(labels_train)}")
    print(f"Shape embeddings: {embeddings_train.shape}")

# Les 3 nombres doivent être identiques !
...
if __name__ == "__main__":
    ...
    pipeline_annotation_vers_modele(
        textgrid_dir=Path(r"B:\drones\test_1\data"),
        audio_dir=Path(r"B:\drones\test_1\data"),
        output_dir=Path(r"B:\drones\test_1\data"),
